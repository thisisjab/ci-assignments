...
func main() {
	jsonData := data.UnmarshalTrainingDataFile("./training_data.json")
	cleanedData := data.PrepareData(jsonData)
	...
	fmt.Printf("Took %v epochs to succeed and final bias is %v\n", epochsToSucceed, bias)
	fmt.Println("Calculated weights:", weights[len(weights) - 1])
